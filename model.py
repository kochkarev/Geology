...
def load_test(imgs_path, masks_path, n_classes, squeeze=True):
    ...
    for p in list(imgs_path.iterdir()):
        img = np.array(Image.open(p)).astype(np.float) / 256
...
model.train(bg.g(), bg.g(), steps_per_epoch=200, epochs=20, validation_steps=20, test_data=test_data, test_overlay=0.0, test_output=exp_path, test_vis=False)
